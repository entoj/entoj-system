 ...
 * Requirements
 */
const ParserPlugin = require(ES_SOURCE + '/model/loader/documentation/ParserPlugin.js')
    .ParserPlugin;
    ...
    describe('#executeFor()', function() {
        ...
        it('should allow to parse a item of a specific site', function() {
            ...
            const promise = testee
                .executeFor(global.fixtures.entityImage, global.fixtures.siteExtended)
                .then(function() {
                    expect(global.fixtures.entityImage.files[0].site).to.be.equal(
                        global.fixtures.siteExtended
                    );
                    expect(global.fixtures.entityImage.documentation[0].site).to.be.equal(
                        global.fixtures.siteExtended
                    );
                });
            return promise;
        ...
        it('should update the site property of all parsed items', function() {
            ...
            global.fixtures.entityTeaser.documentation.clear();
            const promise = testee.executeFor(global.fixtures.entityTeaser).then(function() {
                expect(global.fixtures.entityTeaser.files[0].site).to.be.equal(
                    global.fixtures.siteBase
                );
                expect(global.fixtures.entityTeaser.documentation[0].site).to.be.equal(
                    global.fixtures.entityTeaser.id.site
                );
            });
            return promise;
        });
