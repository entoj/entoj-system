...
describe(EntitiesLoader.className, function() {
    ...
    describe('#load', function() {
        it('should resolve to Entity instances extracted from the given directory structure', function() {
            const testee = new EntitiesLoader(
                global.fixtures.sitesRepository,
                global.fixtures.categoriesRepository,
                global.fixtures.entityIdParser,
                global.fixtures.pathesConfiguration
            ...
            const promise = co(function*() {
                ...
                expect(
                    items.find((item) => item.id.category.longName == 'Global')
                ).to.be.instanceof(Entity);
        ...
        it('should resolve to Entity instances that knows each Site they are used in (via extend)', function() {
            ...
            const promise = co(function*() {
                const items = yield testee.load();
                expect(items.find((item) => item.id.name == 'image').usedBy).to.contain(
