 ...
 * Requirements
 */
const EntityMacrosInheriter = require(ES_SOURCE + '/model/entity/inheriter/EntityMacrosInheriter.js').EntityMacrosInheriter;
...
describe(EntityMacrosInheriter.className, function()
{
    ...
    describe('#inherit', function()
    {
        it('should add all examples of the aspect site', function()
        {
            const docs =
            [
                new DocumentationCallable({ name: 'm1', site: global.fixtures.siteBase, contentKind: ContentKind.MACRO }),
                new DocumentationCallable({ name: 'm1', site: global.fixtures.siteExtended, contentKind: ContentKind.MACRO })
            ];
