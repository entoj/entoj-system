...
describe(UrlsConfiguration.className, function()
{
    ...
    describe('#resolveFilename', function()
    {
        it('should resolve a filename to a url if it exists', function()
        {
            ...
            const promise = testee.resolveFilename(global.fixtures.pathesConfiguration.root + '/base/modules/m-teaser/m-teaser.md').then(function(url)
            {
    ...
    describe('#matchSite', function()
    {
        ...
        it('should allow to match partial pathes with partial=true', function()
        {
            const patterns = { sitePattern: '${root}/:site' };
            const testee = createTestee(patterns);
            const promise = testee.matchSite('/base/elements/m001-gallery', true).then(function(match)
            {
                expect(match.site).to.be.equal(global.fixtures.siteBase);
    ...
    describe('#resolveEntityCategory', function()
    {
        ...
        it('should resolve to the configured path', function()
        {
            const urls = { templateUrl: '${root}/${site.toLowerCase()}/${entityCategory}.longName' };
            const testee = createTestee(urls);
            const promise = testee.resolveEntityCategory(global.fixtures.siteBase, global.fixtures.categoryElement).then(function(url)
            {
    ...
    describe('#matchEntityCategory', function()
    {
        ...
        it('should allow to match partial pathes with partial=true', function()
        {
            const patterns = { entityCategoryPattern: '${root}/:site/:entityCategory' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityCategory('/base/elements/e001', true).then(function(match)
            {
                expect(match.site).to.be.equal(global.fixtures.siteBase);
        ...
        it('should match a direct category path with partial=true', function()
        {
            const patterns = { entityCategoryPattern: '${root}/:site/:entityCategory' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityCategory('/base/module-groups', true).then(function(match)
            {
                expect(match.site).to.be.equal(global.fixtures.siteBase);
                expect(match.entityCategory).to.be.equal(global.fixtures.categoryModuleGroup);
            });
    ...
    describe('#resolveEntityId', function()
    {
        ...
        it('should resolve to the configured path', function()
        {
            const urls = { entityIdTemplate: '${root}/${site.name.toLowerCase()}/${entityCategory.longName}/${entityId.name}' };
            const testee = createTestee(urls);
            const promise = testee.resolveEntityId(global.fixtures.entityTeaser.id).then(function(url)
            {
        ...
        {
            const urls = { entityCategoryTemplate: '${root}/${site.name.toLowerCase()}/${entityCategory.longName.toLowerCase()}' };
            const testee = createTestee(urls);
            const promise = testee.resolveEntityId(global.fixtures.entityGlobal.id).then(function(url)
            {
    ...
    describe('#matchEntityId', function()
    {
        ...
        it('should resolve to the entity when matched', function()
        {
            ...
            const promise = testee.matchEntityId('/base/modules/m-teaser').then(function(match)
            {
                ...
                expect(match.entityId.category).to.be.equal(global.fixtures.entityTeaser.id.category);
            });
            return promise;
        });
        ...
        it('should only match complete pathes', function()
        {
            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityId('/base/modules/m001-gallery/readme.md').then(function(match)
            {
                expect(match.site).to.be.not.ok;
                expect(match.entityCategory).to.be.not.ok;
                expect(match.entityId).to.be.not.ok;
        ...
        it('should allow to match partial pathes with partial=true', function()
        {
            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityId('/base/modules/m-teaser/readme.md', true).then(function(match)
            {
                expect(match.site).to.be.equal(global.fixtures.siteBase);
                expect(match.entityCategory).to.be.equal(global.fixtures.categoryModule);
                expect(match.entityId).to.be.instanceof(EntityId);
    ...
    describe('#matchEntity', function()
    {
        ...
        it('should only match complete pathes', function()
        {
            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntity('/base/modules/m001-gallery/readme.md').then(function(match)
            {
                expect(match.site).to.be.not.ok;
                expect(match.entityCategory).to.be.not.ok;
                expect(match.entityId).to.be.not.ok;
        ...
        it('should allow to match partial pathes with partial=true', function()
        {
            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntity('/base/modules/m-teaser/readme.md', true).then(function(match)
            {
                expect(match.site).to.be.equal(global.fixtures.siteBase);
                expect(match.entityCategory).to.be.equal(global.fixtures.categoryModule);
                expect(match.entityId).to.be.instanceof(EntityId);
    ...
    describe('#matchEntityFile', function()
    {
        ...
        it('should resolve to a entity when matched', function()
        {
            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityFile('/default/modules/m-teaser').then(function(match)
            {
        ...
        {
            const file = new File({ filename: global.fixtures.pathesConfiguration.root + '/base/modules/m-teaser/examples/overview.j2' });
            file.site = global.fixtures.siteBase;
            global.fixtures.entityTeaser.files.push(file);

            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityFile('/base/modules/m-teaser/examples/overview.j2').then(function(match)
            {
        ...
        it('should resolve to a file when matched on a extended aspect', function()
        {
            ...
            const patterns = { entityIdPattern: '${root}/:site/:entityCategory/:entityId' };
            const testee = createTestee(patterns);
            const promise = testee.matchEntityFile('/extended/modules/m-teaser/examples/overview.j2').then(function(match)
            {
                expect(match.file).to.be.ok;
                expect(match.file).to.be.equal(file);
            });
