...
describe(File.className, function() {
    ...
    baseSpec.assertProperty(
        new File({ filename: '/tmp/file.ext' }),
        ['site'],
        new Site('test'),
        undefined
    );
    ...
    baseSpec.assertProperty(new File({ filename: '/tmp/file.ext' }), [
        'basename',
        'path',
        'extension'
    ]);
    ...
    describe('#filename', function() {
        it('should allow to get & set a filename', function() {
            ...
            expect(testee.filename).to.endWith(
                PATH_SEPERATOR + 'tmp' + PATH_SEPERATOR + 'file' + PATH_SEPERATOR + 'name.ext'
            );
        });
