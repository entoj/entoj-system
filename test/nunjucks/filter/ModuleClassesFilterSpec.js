 ...
 * Requirements
 */
const ModuleClassesFilter = require(ES_SOURCE + '/nunjucks/filter/ModuleClassesFilter.js')
    .ModuleClassesFilter;
...
describe(ModuleClassesFilter.className, function() {
    ...
    describe('#filter()', function() {
        ...
        it('should return the moduleClass and a modificator for any given type', function() {
            const testee = new ModuleClassesFilter().filter();
            expect(testee(['default', 'hero'], 'm-teaser')).to.be.equal(
                'm-teaser m-teaser--default m-teaser--hero'
            );
        });
    });
});
