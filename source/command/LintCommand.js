    ...
    lint(parameters, doNotExit) {
        ...
        const promise = co(function*() {
            ...
            for (const entity of entities) {
                const entityPath = yield scope.pathesConfiguration.resolveEntityForSite(
                    entity.entity,
                    entity.site
                );
                const result = {
                ...
                if (result.success) {
                    ...
                    logger.end(
                        ...
                        true,
                        'Linting <' +
                            entity.id.asString('path') +
                            '> failed with ' +
                            chalk.yellow(result.errorCount) +
                            ' errors and ' +
                    ...
                    for (const file in messagesByFile) {
                        ...
                        logger.info(
                            '   ' +
                                messages[0].filename.replace(scope._pathesConfiguration.root, '') +
                                ''
                        );
                        for (const message of messages) {
                            logger.info(
                                chalk.magenta('     @' + message.line) +
                                    ' ' +
                                    message.message +
            ...
            if (sectionResult.errorCount > 0) {
                ...
                if (scope.options.exitCodes !== false && doNotExit !== true) {
                    process.exit(1);
                }
            } else {
                // exit with zero
