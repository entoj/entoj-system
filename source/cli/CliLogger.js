...
class CliLogger extends Base {
    ...
    write() {
        ...
        if (this._showPrefix && this._prefix) {
            ...
            prepend += chalk.gray(
                prefix + ' '.repeat(Math.max(0, this._prefixLength - prefix.length)) + ' '
            );
        }
    ...
    formatMessage(message) {
        message = message.replace(
            /<([^>]+)>/g,
            chalk.styles.cyan.open + '$1' + chalk.styles.cyan.close
        );
        return message;
    }
    ...
    debug() {
        ...
        const message = Array.from(arguments).join(' ');
        this.write(chalk.dim('  ' + this.formatMessage(message)));
    }
