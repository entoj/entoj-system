            ...
            for (const field of node.iterableFields)
            {
                if (Array.isArray(node[field]))
                {
                    ...
                    for (const child of node[field])
                    {
                        try
                        {
                            const transformedNode = yield scope.walk(child, transformation, configuration, level + 1);
                            if (transformedNode)
                            {
                ...
                {
                    node[field] = yield scope.walk(node[field], transformation, configuration, level + 1);
                }
            }
